...
import { useDocsContext, useTheme } from "@/hooks";
...
export default function NavTopBar() {
  ...
  return (
    <nav className="fixed w-full h-14 px-6 py-3 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={() => {
            const themes = Object.values(ThemeOptions);
        ...
        >
          <div className="size-3 flex-shrink-0">
            {themePreference === ThemeOptions.System && <SystemIcon />}
            {themePreference === ThemeOptions.Light && <LightIcon />}
            {themePreference === ThemeOptions.Dark && <DarkIcon />}
          </div>
          {themePreference}
        </button>
        <Link
          ...
          className="hidden sm:flex items-center gap-2 px-3 py-1 text-sm font-medium rounded-xs bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 group relative border border-gray-100 dark:border-gray-700 transition-all duration-300 select-none focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-900"
        >
          <svg
